...
<template>
  ...
  <div class="mt-16">
    <!-- .prevent = impedir que o formulário recarregue/atualizar a página -->
...
export default {
  ...
  setup () {
    ...
    async function login ({ email, password }) {
      ...
      if (!errors) {
        // O token é retornado a partir do POST de LOGIN < para verificar = npm run container (Docker) + Postman
    ...
    async function handleSubmit () {
      try {
        ...
        if (errors.status === 400) {
          toast.errors('Ocorreu um erro ao criar a conta')
        }
